...
function chamberGrid(target, data, party, lean, index, tier, first) {
    ...
    d3.select(target).selectAll(".seat" + first)
        ...
        .call(d3.helper.tooltip(function(d, i) {
            ...
            return "<div class='districtName'>District " + d.seatName + "</div><div>" + status + "</div>" + opponent + "<div class='tipCat'>CPVI</div><div>" + d.cpvi + "</div><div class='hideme'><div class='tipCat'>Current member from</div>" + d.from + "</div><div><div class='tipCat'>Trump margin</div>" + d3.format("+.1f")(d.margin) + "%</div><div class='tipCat'>Special status</div><div>" + d.special_status + "</div>";
        }));
